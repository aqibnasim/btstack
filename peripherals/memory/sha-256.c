...
static void init_buf_state(struct buffer_state * state,
		MemObject *mem, MemChunk *chunk, MemElement *elem, size_t elemoffs,
		size_t len)
{
	//state->p = input;
	state->mem = mem;
...
static void init_buf_state_flash(struct buffer_state * state, const void * input, size_t len)
{
	state->p = input;
	state->len = len;
	state->total_len = len;
...
static int calc_chunk(uint8_t chunk[CHUNK_SIZE], struct buffer_state * state)
{
	...
	if (state->len >= CHUNK_SIZE) {
		...
		memReadFromElement(state->mem, state->chunk, state->elem, state->elemoffs,
				chunk, CHUNK_SIZE);
		state->elemoffs += CHUNK_SIZE;
...
/* Return value: bool */
static int calc_chunk_flash(uint8_t chunk[CHUNK_SIZE], struct buffer_state * state)
{
	size_t space_in_chunk;
		...
		size_t len)
{
	...
	//uint8_t chunk[64];
	uint8_t *chunk = mem->buffer;

	struct buffer_state state;

	init_buf_state(&state, mem, memchunk, elem, elemoffs, len);
...
void calc_sha_256_flash(uint8_t hash[32], const void * input, size_t len)
{
	/*
	 * Note 1: All integers (expect indexes) are 32-bit unsigned integers and addition is calculated modulo 2^32.
	 * Note 2: For each round, there is one round constant k[i] and one entry in the message schedule array w[i], 0 = i = 63
	...
	init_buf_state_flash(&state, input, len);

	while (calc_chunk_flash(chunk, &state)) {
		uint32_t ah[8];
