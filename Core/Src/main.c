...
int main(void)
{
  ...
  serialcomInit(&serialcom, &huart2, &main_state.peripherals,
      		SERIAL_RTOS_PRIORITY, SERIAL_STACKSIZE);
#else
  ...
  //main_state.lowpower_mode = LP_SLEEP;
...
{
	return;		//NOTE: no special clock settings
...
/**
  * @brief Turn on External Crystal
  * @retval None
  */
...
/**
  * @brief Turn ff External Crystal
  * @retval None
  */
void XTALClk_TurnOFF(void)
{
	//HAL_GPIO_WritePin(CLK_EN_GPIO_Port, CLK_EN_Pin, GPIO_PIN_RESET);
}
/* USER CODE END 4 */
